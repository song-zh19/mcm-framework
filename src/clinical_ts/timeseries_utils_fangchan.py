...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def __init__(self, df, output_size, chunk_length, min_chunk_length, memmap_filename=None, npy_data=None, random_crop=True, data_folder=None, num_classes=2, copies=0, col_lbl="label", stride=None, start_idx=0, annotation=False, transforms=None, sample_items_per_record=1, multi_modal_dim=None):
        """
        ...
        for df_idx,(id,row) in enumerate(tqdm(df.iterrows(), total=df.shape[0], ncols=120)):
            if(self.mode=="files"):
            ...
            for _ in range(copies+1):
                for i_s,i_e in zip(idx_start,idx_end):
                    ...
                    self.file_mapping.append(row['data_original'])
        ...
        import json
        self.inames = json.load(open("/home/user/code/stat/logs/inames.json"))
        self.workno2zhibiao = json.load(open("/home/user/code/stat/logs/workno2zhibiao_new.json"))
        self.filename2workno = json.load(open("/home/user/code/stat/logs/filename2workno.json"))
    ...
    def _getitem(self, idx,start_idx_rel):
        ...
        original_filename = self.file_mapping[idx]
        #determine crop idxs
        ...
        if(self.mode=="files"):#from separate files
            data_filename = str(self.timeseries_df_data[df_idx]) #todo: fix potential issues here
            # print(data_filename)
            if self.data_folder is not None:
                data_filename = self.data_folder/data_filename[3:]
            ID = data_filename.stem

            if '@' in str(data_filename):
                data_filename, data_idx = str(data_filename).split('@')
                data_start_idx, data_end_idx = data_idx.split('_')
                if data_filename not in self.cache_data.keys():
                    sigbufs = np.loadtxt(data_filename, dtype=np.float32)
                    ...
                    if sigbufs.min() == 0 and sigbufs.max() == 4095:
                        ...
                        if data_filename.split('/')[-4] == 'Quzhou':
                            sigbufs = sigbufs
                        elif data_filename.split('/')[-4] == 'Anzhen':
                            sigbufs = sigbufs - 2048
                        else:
                            print(data_filename)
                    
                    sigbufs = resample_data(sigbufs=sigbufs, 
                    ...
                    self.cache_data[data_filename] = sigbufs
                
                data = self.cache_data[data_filename][int(data_start_idx)//4:int(data_end_idx)//4]
                # data = os.popen('sed -n {},{}p {}'.format(data_start_idx, data_end_idx, data_filename)).read()
                # data = np.array(data.split('\n')[:-2]).astype(np.float32)[start_idx_crop:end_idx_crop] 
            ...
            if self.multi_modal_dim is None:
                modaldata = np.zeros((1, 97))
            else:
                # modaldata = np.load(self.memmap_meta_filename.parent/'qwen2Embedding'/original_filename)
                modaldata_all = np.load(self.memmap_meta_filename.parent/'qwen2Embedding_with_LLM'/original_filename)
                ordered_features = [39,79,1,2,15,80,27,10,57,85,54,5,64,0,20,11,44,42,71,36,14,26,21,75,33,58,78,45,8,6,77,47,72,51,49,83,19,56,53,30,3,37,29,12,34,82,55,18,17]
                workno = self.filename2workno[str(original_filename)[:-4]]
                zhibiao_iname = self.workno2zhibiao[workno]['iname']
                modaldata_num = 0
                modaldata = []
                modaldata.append(modaldata_all[0])
                modaldata.append(modaldata_all[1])
                for feature_idx in ordered_features:
                    iname = self.inames[feature_idx]
                    if iname in zhibiao_iname:
                        modaldata_num += 1
                        modaldata.append(modaldata_all[zhibiao_iname.index(iname) + 2])
                        if modaldata_num == 20:
                            break
                modaldata.append(modaldata_all[-2])
                modaldata.append(modaldata_all[-1])
                modaldata = np.array(modaldata)

                # relevant_features = [39,79,1,2,15,80,27,10,57,85,54,5,64,0,20,11,44,42,71,36]   # most relevant features
                # used_inames = [self.inames[iname_idx] for iname_idx in relevant_features]
                # workno = self.filename2workno[str(original_filename)[:-4]]
                # zhibiao_iname = self.workno2zhibiao[workno]['iname']
                # modaldata = []
                # modaldata.append(modaldata_all[0])
                # modaldata.append(modaldata_all[1])
                # for iname_idx in range(len(zhibiao_iname)):
                #     modaldata_idx = iname_idx + 2
                #     if zhibiao_iname[iname_idx] in used_inames:
                #         modaldata.append(modaldata_all[modaldata_idx])
                # modaldata.append(modaldata_all[-2])
                # modaldata.append(modaldata_all[-1])
                # modaldata = np.array(modaldata)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        modaldata = _to_tensor(modaldata, False)
        ...
        return data, modaldata, label #returning as a tuple (potentially of lists)
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        if(self.input):
            return (data, modaldata, labelx)
        else:
            return (datax, modaldata, data)
