...
def multiclass_roc_curve(y_true, y_pred, classes=None, precision_recall=False,):
    ...
    if n_classes == 1:
        # y_pred_neg = 1 - y_pred
        # y_pred = np.concatenate([y_pred_neg, y_pred], axis=1)
        # y_true_neg = 1 - y_true
        # y_true = np.concatenate([y_true_neg, y_true], axis=1)
        # n_classes = 2
        classes = classes[1:]
    if(classes is None):
...
# Cell
def eval_scores(y_true, y_pred, classes=None, acctype='single', num_thresholds=100, full_output=False, parallel=True):
    ...
    * Fmax, sample AUC, sample Average Precision (as in sklearn)
    ...
    if acctype == 'multi':
        label_pred = y_pred.copy().astype(np.float32)
        label_pred[label_pred >= 0.7] = 1
        label_pred[label_pred < 0.7] = 0

        label_pred = np.sum(label_pred == y_true, axis=1) # 统计每个样本有多少个类是正确的
        correctNum = np.sum(label_pred == y_true.shape[1]) # 所有类都正确算是预测对了
        results["acc"] = correctNum / y_true.shape[0]
